...
async fn create_transfer(
    ...
    Ok(Json(CreateTransferResponse {
        ...
        token,
        qr_url,
        expires_at: expires_at.map(|dt: chrono::DateTime<chrono::Utc>| dt.to_rfc3339()),
    }))
}
...
async fn get_transfer(
    ...
    let file = state.storage.get(&id)
        .await
        .ok_or_else(|| (StatusCode::NOT_FOUND, "File not found or expired".to_string()))?;
...
async fn delete_transfer(
    ...
    if deleted {
        ...
        Err((StatusCode::NOT_FOUND, "File not found or already expired".to_string()))
    }
}
...
// #[cfg(test)]
// mod tests {
//     // API tests - to be implemented with proper tower ServiceExt setup
// }
